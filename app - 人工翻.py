...
def apply_custom_theme():
    st.markdown(
        ...
        /* 去掉 Streamlit 在每个 element-container 上加的默认 vertical gap */
...
def render_bgnbd_module(data_dir: str = ".") -> None:
    ...
    if dataset_mode == "Default dataset (events.csv)":
        ...
        freq_threshold = st.slider("High-Value Threshold: Frequency (Minimum Purchase Count)", min_value=0, max_value=freq_max, value=min(default_freq, freq_max), step=1)
        alive_threshold = st.slider("Churn Alert Threshold: P(Alive) Upper Limit (Lower is More Dangerous)", min_value=0.0, max_value=1.0, value=0.5, step=0.05)
    ...
    if rfm.empty:
        st.warning("Current data contains no transaction events, BG/NBD analysis cannot be performed.")
        return
...
<span style="font-family:'Playfair Display',Georgia,serif;font-size:1.05rem;font-weight:700;color:#E2B659;letter-spacing:0.06em">Modules</span>
</div>""",
        unsafe_allow_html=True,
    )
    _module_options = [
