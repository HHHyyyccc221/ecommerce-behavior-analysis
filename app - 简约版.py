...
def render_reco_module(data_dir: str = ".") -> None:
    ...
    # 跳转时预设 Manual Input；之后用户可自由切换，不再干预
    if st.session_state.pop("jump_force_manual", False):
        st.session_state["_reco_user_mode"] = "Manual Input"
    ...
    # 跳转时将 visitorid 写入 key，之后用户在文本框编辑时 Streamlit 自行维护
    ...
    if user_mode == "Example User":
        ...
        visitorid = st.sidebar.text_input(
            "Manual Input",
            key="_manual_input_id",
        ).strip()
        if not visitorid:
    ...
    algo_mode = st.sidebar.radio(
        ...
        format_func=lambda x: {
            "FPMC": "FPMC (均等权重，无时间衰减)",
            "TD-Multifaceted-FPMC": "TD-Multifaceted-FPMC (时间衰减 e⁻λΔt)",
        }.get(x, x),
        index=1,
    )
...
def render_intent_prediction_module() -> None:
    ...
    with col_left:
        ...
        if st.button("立即预测", type="primary"):
            ...
            # 展示层启发式修正：浏览次数极低且无加购时，概率上限压低
            # 防止 SMOTE 引入的"低浏览→高概率"反直觉输出
...
def main():
    ...
    # 跳转时仅在 widget 首次渲染前写入 key，后续 Streamlit 自行管理
