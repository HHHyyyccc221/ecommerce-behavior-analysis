...
def load_behavior_cache() -> dict:
    """加载 prepare_behavior_cache.py 生成的离线缓存，缺失时返回 None。"""
...
def load_bgnbd_cache() -> dict:
    """
    从 bgnbd_cache/ 加载全部预计算产物，返回 dict。
    缺失文件时对应 key 为 None，调用方负责降级处理。
    运行 prepare_bgnbd_cache.py 生成缓存。
    """
    import json as _json

    def _p(name):
        return os.path.join(BGNBD_CACHE_DIR, name)
...
def load_funnel_cache() -> dict:
    """加载 prepare_funnel_cache.py 生成的类目转化漏斗缓存。"""
...
def _render_funnel_section() -> None:
    ...
    if cat_df is None:
        st.warning(
            "未找到漏斗缓存，请先运行：`python prepare_funnel_cache.py`"
        )
        return
    ...
    if item_df is not None:
        st.markdown("##### 下钻：查看指定类目内各物品转化率")
        cat_options = top_cats["categoryid"].astype(str).tolist()
        selected_cat = st.selectbox("选择类目", options=cat_options,
                                    format_func=lambda x: f"类目 {x}")
        items_in_cat = item_df[item_df["categoryid"].astype(str) == selected_cat].copy()
        items_in_cat = items_in_cat[items_in_cat["view_cnt"] >= 5]  # 过滤低频物品
        items_in_cat = items_in_cat.sort_values(metric, ascending=False).head(20)

        if items_in_cat.empty:
            st.caption("该类目下无足够浏览量的物品数据。")
        else:
            items_in_cat["rate_pct"] = (items_in_cat[metric] * 100).round(2)
            items_in_cat = items_in_cat.sort_values("rate_pct", ascending=True)
            fig2 = go.Figure()
            fig2.add_trace(go.Bar(
                x=items_in_cat["rate_pct"],
                y=items_in_cat["itemid"].astype(str),
                orientation="h",
                text=items_in_cat["rate_pct"].apply(lambda v: f"{v:.2f}%"),
                textposition="outside",
                marker_color="#F59E0B",
            ))
            fig2.update_layout(
                xaxis_title="转化率 (%)",
                yaxis_title="物品 ID",
                height=max(280, len(items_in_cat) * 26),
...
def _load_user_events(visitorid: str, data_dir: str) -> Optional[pd.DataFrame]:
    """从 events.csv 分块读取目标用户的行为记录，清洗后返回。找不到则返回 None。"""
...
def render_reco_module(data_dir: str = ".") -> None:
    ...
    # ── 加载离线缓存（转移表 + top_users 列表，precomputed_recs 不再用于输出）──
    ...
    if user_mode == "Example User":
        ...
        _prefill = st.session_state.pop("jump_to_reco_user", "")
        visitorid = st.sidebar.text_input("Manual Input", value=_prefill).strip()
        if not visitorid:
    ...
    # ── 读取用户行为记录（两种模式统一走实时读取，不依赖 precomputed_recs）────
    ...
    # ── 调用推荐函数（alpha/gamma/use_time_decay 由算法版本决定）─────────────
...
def render_bgnbd_module(data_dir: str = ".") -> None:
    ...
    # ══════════════════════════════════════════════════════════════════════════
    # 路径 A — 默认数据集：直接加载离线预计算缓存，无需实时清洗/训练
    # ══════════════════════════════════════════════════════════════════════════
    if dataset_mode == "Default dataset (events.csv)":
        cache = load_bgnbd_cache()

        missing_keys = [k for k in ("rfm", "bgf", "heatmap_matrix", "heatmap_meta", "p_alive")
                        if cache[k] is None]
        if missing_keys:
            st.warning(
                f"缓存文件不完整（缺少：{', '.join(missing_keys)}）。\n\n"
        ...
        # 热力图（使用预计算矩阵，直接 imshow，与 lifetimes 原版方向一致）
        ...
        try:
            ...
            # x = recency (t_x)，y = frequency；origin="lower" 使 y 轴从下到上递增
            # x 轴翻转：左=recency 大（最近有购买）→ 右=recency 小（很久没买）
            # 与 lifetimes 原版热力图方向一致，左上角红色 = 高频+近期活跃
            im = ax.imshow(
                heatmap_matrix,
                aspect="auto",
                origin="lower",
                extent=[recency_range[-1], recency_range[0],
                        freq_range[0],     freq_range[-1]],
                cmap="coolwarm",
            )
            fig.colorbar(im, ax=ax, label="Expected Purchases (next 1 period)")
            ax.set_xlabel("Recency: days since last purchase (t_x)")
            ax.set_ylabel("Frequency (repeat purchases)")
            ax.set_title("BG/NBD: Expected Transactions (Recency vs Frequency)")
        ...
        st.markdown(
            "**物理意义**（x 轴从右到左 t_x 增大，即右侧=最近刚买、左侧=很久没买）：\n"
            "- **右上（高 Frequency + t_x 小，最近刚买）**：核心活跃客户，购买意愿最强。\n"
            "- **左上（高 Frequency + t_x 大，很久没买）**：历史高贡献但近期沉默，流失风险高（重点预警）。\n"
            "- **右下（低 Frequency + t_x 小）**：新客，近期有购买但历史浅。\n"
            "- **左下（低 Frequency + t_x 大）**：低价值或已流失客户。"
        )
        ...
        freq_threshold = st.slider(
            ...
            min_value=0, max_value=freq_max,
            value=min(default_freq, freq_max), step=1,
        )
        ...
        if not alert.empty and "visitorid" in alert.columns:
            ...
            selected_churn_user = st.selectbox(
                "选择流失风险用户", options=churn_users,
                key="churn_to_reco_user"
            )
    ...
    # ══════════════════════════════════════════════════════════════════════════
    # 路径 B — 上传自定义数据集：实时清洗 + 训练（保持原有逻辑）
    # ══════════════════════════════════════════════════════════════════════════
    uploaded = st.sidebar.file_uploader(
        "上传 CSV（需包含 visitorid/event/timestamp 或可自动适配的列）", type=["csv"]
    )
    if uploaded is None:
    ...
    with st.spinner("正在拟合 BG/NBD 模型…"):
...
def render_intent_prediction_module() -> None:
    ...
    st.sidebar.caption("基于当前会话的浏览次数、加购次数、停留时长等，反映本次访问的即时意图。")
    ...
    # 用 session_state 保存预测结果，使左右两列同步刷新
    ...
    with col_right:
        if st.session_state["m1_prob"] is not None:
            render_behavior_marketing(
                st.session_state["m1_prob"],
                st.session_state["m1_behavior"],
            )
        else:
            st.markdown("<div style='margin-top:60px'></div>", unsafe_allow_html=True)
            st.markdown("**精细营销建议**")
...
def main():
    ...
    # 处理从模块二跳转过来的联动请求
    if st.session_state.get("jump_to_module"):
        module = st.session_state.pop("jump_to_module")
