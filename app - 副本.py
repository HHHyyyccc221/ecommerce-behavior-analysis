...
def select_quality_top_users(
    ...
    """
    在所有活跃用户（行为数 > 5）中，用改进版 TD-Multifaceted-FPMC 计算 Top-1 匹配概率，
    精确筛选出 Top-1 概率最高的前 10 位作为“优质示例用户”。
    """
    vc = events["visitorid"].astype(str).value_counts()
    eligible = vc[vc >= min_actions].index.tolist()
    if not eligible:
        return []
    ...
    for uid in eligible:
        ...
        top1 = float(recs[0][2]) if recs else 0.0
        scored.append((uid, top1))

    scored.sort(key=lambda x: x[1], reverse=True)
    return [u for u, _ in scored[:10]]
...
def recommend_td_multifaceted_fpmc(
    ...
    """
    TD‑Multifaceted‑FPMC 近似实现：
      Score = alpha(用户偏好) + beta(时间加权转移) + gamma(类别关联)
    返回 top-k：[(itemid, reason, prob)]
    """
    ...
    seen = set(u_ev["itemid"].astype(str).unique().tolist())
    scores: Dict[str, float] = {}
    reasons: Dict[str, str] = {}

    # 1) 时间加权转移（item->next_item）
    for item_i, ts_i in last3:
        item_i = str(item_i)
        if use_time_decay:
            dt = float(t_last - float(ts_i))
            w = float(np.exp(-LAMBDA_DECAY * dt))
        else:
            w = 1.0
    ...
    # 2) 类别关联（category->next_category，再落到具体 item）
    for item_i, ts_i in last3:
        item_i = str(item_i)
        ...
        for next_cat, p in cat_next.get(str(cat_i), [])[:20]:
            cand_items = cat_to_items.get(str(next_cat), [])[:80]
            if not cand_items:
                continue
            # 将类别概率均匀分摊到该类别下的热门 item（近似）
            share = float(p) / max(len(cand_items), 1)
    ...
    # 兜底：如果候选太少，加一些热门 item
    if len(scores) < k:
        for it in global_hot:
            if it in seen or it in scores:
                continue
            scores[it] = scores.get(it, 0.0) + 1e-6
    ...
    for it, prob in top:
        reason = reasons.get(it, "hot")
        reason_text = "seq" if reason == "seq" else ("cat" if reason == "cat" else "hot")
        out.append((it, reason_text, float(prob)))
    return out[:k]
...
def render_reco_module(data_dir: str = ".") -> None:
    ...
    events = load_events_for_reco_default(data_dir=data_dir)
    ...
    # 侧边栏：示例用户 / 自定义输入 + 算法版本
    top_users = select_quality_top_users(
        events=events,
        item_to_cat=item_to_cat,
        item_next=item_next,
        cat_next=cat_next,
    ...
    if not top_users:
        top_users = events["visitorid"].astype(str).value_counts().head(10).index.tolist()
    ...
    if user_mode == "示例用户":
        visitorid = st.sidebar.selectbox(
            "示例用户（按行为数排序）",
            options=top_users,
        )
    else:
        visitorid = st.sidebar.text_input("请输入特定的 VisitorID").strip()
        if not visitorid:
            return

    if visitorid not in set(events["visitorid"].astype(str).unique()):
        st.sidebar.error("该 ID 在数据集中不存在，请重新输入")
        return

    mode = st.sidebar.radio(
        "推荐算法版本",
        options=["基础版 FPMC", "改进版 TD-Multifaceted-FPMC"],
        index=1,
    )
    is_basic = mode == "基础版 FPMC"
    ...
    # 注意：recs 的 prob 来自 softmax，Top-5 概率和恒为 1，因此“平均概率”恒为 20%。
    # 为体现不同用户的匹配差异，这里展示 Top-1 匹配概率作为置信度。
    top1_prob = float(recs[0][2]) if recs else 0.0
    st.metric("Top-1 匹配概率", f"{top1_prob:.2%}")
    ...
    for i in range(5):
        with cols[i]:
            ...
            # 精简 Reason，避免超出卡片宽度
            label = "Seq" if reason == "seq" else ("Cat" if reason == "cat" else "Hot")
