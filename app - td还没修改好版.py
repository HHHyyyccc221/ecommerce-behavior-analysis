...
def render_reco_module(data_dir: str = ".") -> None:
    ...
    user_mode = st.sidebar.radio(
        "User Source",
        options=["Pre-warmed Examples (Offline)", "Manual ID (Real-time)"],
        index=0,
    )
    ...
    algo_mode = st.sidebar.radio(
        "Algorithm Version",
        options=["Basic FPMC", "Improved TD-Multifaceted-FPMC"],
        index=1,
    )
    is_basic       = algo_mode == "Basic FPMC"
    ...
    # ═══════════════════════════════════════════════════════════════════════════
    if user_mode == "Pre-warmed Examples (Offline)":
        if not has_cache:
            st.warning(
        ...
        visitorid = st.sidebar.selectbox(
            "Select Example User (Top-10 by confidence)",
            options=top_users,
        )
    ...
    # ═══════════════════════════════════════════════════════════════════════════
    # MODE B — Manual ID: chunked CSV read + real-time FPMC inference
    # ═══════════════════════════════════════════════════════════════════════════
    else:
        visitorid = st.sidebar.text_input("Enter Visitor ID").strip()
        if not visitorid:
